...
vi.mock("./task");
vi.mock("./generate-in-between-days");

beforeEach(() => {
  vi.resetAllMocks();
  vi.useFakeTimers();
  vi.mocked(generateInBetweenDays).mockImplementation((days) => days);
});
...
test("Planning engine generates days from generateInBetweenDays", () => {
  ...
  when(generateInBetweenDays)
    .calledWith([dayOne, dayTwo])
    .mockReturnValue([dayTwo, dayOne]);

  const engine = new PlanningEngine([dayOne, dayTwo]);

  expect(engine.days).toEqual([dayTwo, dayOne]);
});
